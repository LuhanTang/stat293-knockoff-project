...
```{r}
#############################################
#   STAT 293 – Model-X Knockoffs Project
...
############################################################
### 3. Data generators (Gaussian only)
############################################################
...
############################################################
### 4. Lasso-based statistic for Knockoffs (Gaussian only)
############################################################
lasso_stat_generic <- function(X, Xk, y){
  Xaug <- cbind(X, Xk)
  fit  <- glmnet(Xaug, y, family = "gaussian")
  
  # For each lambda, calculate RSS and select the smallest.
  yhat <- predict(fit, Xaug, type = "response")  # n x length(lambda)
  rss  <- colSums((y - yhat)^2)
  idx_min <- which.min(rss)
  
  # Take the corresponding column directly from the beta matrix (no intercept).
...
############################################################
### 5. Knockoff+ selection (Gaussian only)
############################################################
run_knockoff <- function(X, y, q = 0.10){
...
############################################################
### 6. BH baseline (Gaussian only)
############################################################
run_bh <- function(X, y, q = 0.10){
...
############################################################
### 7. Single simulation for a chosen Gaussian model
############################################################
one_run_model <- function(model = c("gaussian", "gaussian_lowdim"),
                          q = 0.10, seed = 1){
